---
description: "TokenRouter rollout 的 Web 界面，面向配置 composer rollout 按钮与 judge 端点设置页的用户和维护者。"
...
本包为 Web GUI 提供两处 rollout 界面：发起一轮 rollout 的 composer 按钮，以及填写 judge 端点与轮次规模的设置页。当部署希望用户从 GUI 而不是手敲 `/rollout` 来驱动 TokenRouter rollout 时挂载它；轮次本身、其会话事件，以及被 steering（中途引导）送入的获胜方案，仍归 host 插件 `@deepseek-ai/dsh-tokenrouter-rollout` 所有。部署唯一不能跳过的字段是 `judgeBaseURL` —— 随包发布的组合都不携带 judge 端点，因此这个设置页就是用户填入自有 OpenAI 兼容 URL 的地方。两处界面都只做读取与派发。
...
`RolloutStatsPanel` 不占任何座位。它此前填在 `conversation.details.footer` 上，而上游 0.1.5-alpha.1 连同声明该座位的会话详情面板一起下线了它；组件本身与它读取的 `rolloutStats` 投影都完好，等待改绑到一个仍然存在的座位。
...
按钮的注入面只带一个动作 `run`，它经 `ctx.remote.commands.execute` 派发 `/rollout`，并把准入失败映射为一行用户可见提示。设置页的注入面带 `set` 与 `unset`，两者都直接写穿 scope，因此 host 会实时生效。统计面板完全不需要注入面：它经会话标准工具包的 `useProjection` 读取 `rolloutStats` 投影，在尚无 rollout 时返回 `null`。

| 文件 | 职责 |
|---|---|
| `src/client/index.ts` | 插件主体：locale 注册、scope 镜像、两处 slot 注册。 |
| `src/client/RolloutButton.tsx` | composer 按钮。 |
| `src/client/RolloutSettings.tsx` | 设置页。 |
| `src/client/RolloutStatsPanel.tsx` | 统计读数，按会话标准工具包定型；当前未挂载。 |
...
- **统计读数未挂载** —— 上游 0.1.5-alpha.1 下线了 `conversation.details.footer`，因此没有任何位置渲染 `RolloutStatsPanel`；用户只能经 host 读取轮次统计。把它改绑到一个仍然存在的座位是暂缓事项。
- **统计是整会话读数** —— 单轮明细（每轮的分数与获胜方）暂缓实现；投影目前只暴露聚合值。
