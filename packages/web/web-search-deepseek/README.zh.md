...
| `stream` | `false` | 将响应按 Messages 事件流读取，而非单个 JSON 响应体，并重组出相同的块。当某个端点的单次响应携带的 `web_search_tool_result` 块没有 `content` 时启用它——见[只在流上作答的端点](#endpoints-that-answer-only-on-the-stream) |
...
有些 Anthropic 兼容网关会执行服务端搜索，但在单次响应中返回一个**没有 `content`** 的 `web_search_tool_result` 块，只在事件流上给出结果与引用摘录。面对这类端点，默认模式找到的结果块中没有任何可引用内容，搜索会报告没有来源——读起来像是一个空的 web，而不是一处协议差异。此时设置 `stream: true`：提供方会带上 `Accept: text/event-stream` 发送 `stream: true`，从帧中重组 `web_search_tool_result` 块与 `citations_delta` 摘录，并按处理单次响应体的方式完全一致地映射它们——包括严格模式、去重与 `WEB_ABORTED` 取消。官方 DeepSeek 在单次响应中返回完整的块，因此默认值保持为 `false`。

流式只影响响应的读取方式，并不会让搜索变成增量的：seam 仍然在流结束后返回一份完整结果。
...
- **`stream` 是手工设置项，而非探测所得**——协议上无法区分「省略了 `web_search_tool_result.content` 的端点」与「确实没有搜到结果」，因此提供方既不能自行选择模式，也不能跨模式重试。取值错误会表现为搜索报告没有来源。
