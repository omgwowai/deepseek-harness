...
# 目标：<SSH 别名>:<远端目录>（按实际部署配置，见下方三个变量）
# 用法：
...
SSH_HOST="${SSH_HOST:-your-bastion-host}"
REMOTE_ROOT="${REMOTE_ROOT:-/remote/path/deepseek-harness-session-record}"
DSH_HOME_LOCAL="${DSH_HOME:-/Users/lbc/Documents/dsh-v2/dsh-home}"
WORK_ROOT="${WORK_ROOT_OVERRIDE:-/Users/lbc/Documents/dsh-v2}"
PY="/usr/bin/python3"
NOTIFY="${DSH_SKILLS_DIR:-/Users/lbc/Documents/dsh-v2/dsh-home/skills}/feishu/scripts/feishu-notify.py"
STATE_DIR="${DSH_SKILLS_DIR:-/Users/lbc/Documents/dsh-v2/dsh-home/skills}/session-cloud-sync/state"
...
case "${1:-}" in
  ...
  sync)
    if do_sync; then
      ...
      log "sync OK"
      notify "DSH 日报｜✅ 会话同步成功（12:30）｜本地会话已增量复制到 $SSH_HOST:$REMOTE_ROOT（本地保留，会话数据 ${total}）"
      echo "sync OK"
    else
      log "sync FAILED"
