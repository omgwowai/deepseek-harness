...
- 群消息推送：`$HOME/.dsh/feishu/feishu-notify.py`（群机器人 webhook 由使用者自行注册，配置在 `$HOME/.dsh/feishu/webhook.conf`）

```bash
LARK=$HOME/Documents/harness-rc8/tooling/lark-cli/node_modules/.bin/lark-cli
NOTIFY="/usr/bin/python3 $HOME/.dsh/feishu/feishu-notify.py"
```
...
4. 把 Webhook 地址写入本地 `$HOME/.dsh/feishu/webhook.conf`（权限 600，不入库）。
