...
服务台由常驻进程托管（启动脚本：`${DSH_SERVICE_HUB_DIR:-/Users/lbc/Documents/dsh-v2/service-hub}/start-hub.sh`，端口 `DSH_SERVICE_HUB_PORT` 可覆盖，默认 6692）。记录实例、会话、服务名称、URL、端口、工作目录、PID、启动命令、状态与错误；并后台轮询 K8s 集群 / SSH 主机，把登记任务与实际负载做 regex 匹配展示。
...
```bash
HUB=${DSH_SERVICE_HUB_CLI:-${DSH_SERVICE_HUB_DIR:-/Users/lbc/Documents/dsh-v2/service-hub}/scripts/service_hub.py}
...
- 面板不可达时：先 `python3 $HUB health`；仍失败则检查常驻进程（`${DSH_SERVICE_HUB_DIR:-/Users/lbc/Documents/dsh-v2/service-hub}/start-hub.sh`）是否在运行。
