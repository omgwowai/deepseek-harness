...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const strongStyle = { color: 'var(--dsh-text-secondary, #8b98a9)', fontWeight: 650 }

    function DockComponent({ remote, sessionId }) {
      ...
      // 始终并列显示「本会话」与「今日总开销」：无会话用量时本会话回退 ¥0.00。
      const sessions = Array.isArray(state.sessions) ? state.sessions : []
      const session = sessions.find((s) => s !== null && s !== undefined && s.id === sessionId) ?? null
      const today = state.today ?? {}
      const sessionTokens = session !== null
        ? `${fmtTokens(session.input)} in · ${fmtTokens(session.cacheRead)} cache · ${fmtTokens(session.output)} out`
        : ''
      return createElement('div', { style: dockStyle, title: 'Token Router 本会话与今日总开销' },
        createElement('span', null, '本会话 ',
          createElement('span', { style: strongStyle }, session !== null ? fmtMoney(session.cost) : '¥0.00'),
          sessionTokens ? createElement('span', null, ' ' + sessionTokens) : null),
        createElement('span', { style: { color: 'var(--dsh-text-secondary, #8b98a9)' } }, '|'),
        createElement('span', null, '今日总开销 ',
          createElement('span', { style: strongStyle }, fmtMoney(today.cost)),
    ...
    async function apply(ctx) {
      ...
      // 会话内 composer dock 有 sessionId（session 域）；侧边栏 footer 是 root 域、无会话参数。
      const injectedDock = (sessionId) => ({ remote: cost, sessionId })
      const injectedFooter = () => ({ remote: cost })
