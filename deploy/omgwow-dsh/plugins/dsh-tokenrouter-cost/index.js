...
const service = {
  ...
  /**
   * 供其它插件把绕过 ctx.llm 的直连用量补记到账本（例如 rollout 的 judge 走
   * fetch 直连 /chat/completions，不经过 llm/stream 包装器，因此不会被自动计费）。
   * buckets: { input, output, cacheRead, cacheWrite }（token 数）。
   */
  accountUsage(buckets, model, sessionId) {
    account(buckets, model, sessionId, Date.now())
  },
}
