...
DEFAULT_CONFIG = {
    # 轮询的 SSH 主机取自 ~/.ssh/config 的 Host 别名。这里只放示例，
    # 实际部署按自己的机器填写；留空数组即关闭 SSH 来源。
    "ssh_hosts": [
        {"name": "example-bastion", "enabled": True},
        {"name": "example-compute", "enabled": False, "note": "示例：禁用项"},
    ],
    "k8s_clusters": [
...
# 会话日志文件名随 dsh 版本演进：0.1.2-alpha.5 为 session.jsonl.zstd，
# 0.1.5 起把格式版本内嵌进文件名，变成 session.v3.jsonl.zstd。旧写法只匹配前者，
# 结果新版实例的会话在服务台里整体"消失"（会话列表为空、输入统计为 0）。
# 这里按名字精确匹配两代命名，后续 v4/v5 同样适用。
...
def derive_title(session_id: str, stored_title: Optional[str]) -> str:
    ...
    for root in _scan_roots():
        for log_path in _session_logs(root):
...
def _scan_sessions() -> list[dict[str, Any]]:
    ...
    for root in _scan_roots():
        if not root.is_dir():
            continue
        for log_path in sorted(_session_logs(root)):
...
def _database_key() -> Optional[tuple[Any, ...]]:
    ...
    for root in _scan_roots():
        if not root.is_dir():
            continue
        for path in _session_logs(root):
            try:
                stat = path.stat()
            except OSError:
                continue
