...
Each model's `input` list records the **empirically verified** modality, not the advertised one. Six routes on this key accept an image part and silently drop it; `deepseek-v3.2` is declared `input: [text]` for that reason. Route-level `defaultInput: [text]` keeps an undeclared model blind, so a model added later fails closed. `maxRequestImageBytes: 12582912` stays under the gateway's body cap, below the 20 MiB default the [request image bound](../bug-fix/2026-08-18-request-image-payload-bound.md) sets.
